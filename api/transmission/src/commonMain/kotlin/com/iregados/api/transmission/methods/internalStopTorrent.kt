...
suspend fun TransmissionApi.internalStopTorrent(ids: List<String>): ApiResult<Boolean> {
    val request = TorrentStopRequest(
        arguments = TorrentStopRequest.Arguments(ids.map { it.toInt() })
    )
